        ...
        <div class="container-fluid">
            ...
            <div class="panel panel-headline">
                <div class="panel-heading">
                    <h3 class="panel-title">User - {{$user->name}} <small><i class="fa fa-arrow-circle-left"></i><a href="{{route('allUsers')}}">To List</a></small></h3>
                </div>
                <div class="panel-body">
                    ...
                    <div class="row">
                            ...
                            <form class="form-horizontal" action="{{url('postEditUser/' . $user->id)}}" method="post" enctype="multipart/form-data">
                                ...
                                <div class="col-sm-10">
                                  <select class="form-control" name="role_id" id="role_id">
                  			             @foreach($roles as $item)
                    			              <option value="{{$item->id}}" @if($item->id == $user->role_id) selected="selected" @endif>{{$item->role_name}}</option>
                                     @endforeach
                                   </select>
                                 </div>
                              </div>
                            ...
                            -->
                              ...
                              <div class="form-group">
                                ...
                                <div class="col-sm-10">
                                  <input type="text" class="form-control" id="mobile" name="mobile" placeholder="Phone No" value="{{$user->mobile}}">
                                </div>
                              </div>
                              <div class="form-group">
                                ...
                                <div class="col-sm-10">
                                  <select class="form-control" name="location_id" id="location_id">
                  			             @foreach($locations as $item)
                    			              <option value="{{$item->pickup_id}}" @if($item->pickup_id == $user->location_id) selected="selected" @endif>{{$item->name}}</option>
                                     @endforeach
                                   </select>
                                 </div>
                              </div>
